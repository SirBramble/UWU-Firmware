...
void module::updateKeymapsFromFile(){
  ...
  Serial.println("running second while");
    ...
    if(lineSearch.Match("^([ \t]*)%%", 0) == REGEXP_MATCHED){}
      ...
      layerCurrent = atoi(lineSearch.GetCapture(buf, 1));
      Serial.printf("found COLOR MODE on layer: %d\n", layerCurrent);
      buf_effect += lineSearch.GetCapture(buf,3);
      ...
      buf_effect += lineSearch.GetCapture(buf,3);
      buf_color = string_to_color(lineSearch.GetCapture(buf,4));
      ...
      buf_effect += lineSearch.GetCapture(buf,3);
      buf_color = string_to_color(lineSearch.GetCapture(buf,4));
      buf_speed = atoi(lineSearch.GetCapture(buf, 5));
      ...
      Serial.printf("current Layer: %d", layerCurrent);
