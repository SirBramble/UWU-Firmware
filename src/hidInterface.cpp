...
uint8_t midi_CC_state_storage[16][256];
  ...
  if(value != midi_value_storage[inputKey->MIDI_channel][inputKey->MIDI_data1]){
    ...
    switch (inputKey->MIDI_mode) {
      case MIDI_CC:
        ...
        Serial.printf("Sending MIDI CC value: %d on CC_NUM %d and channel %d\n", value, inputKey->MIDI_data1, inputKey->MIDI_channel);
        break;
      default:
        break;
    }
...
uint8_t hidInterface::get_midi_CC_state(uint8_t channel, uint8_t number){
  return midi_CC_state_storage[channel][number];
}

uint8_t hidInterface::get_midi_CC_state(key * inputKey){
...
void hidInterface::clear_midi_CC_update_available(){
  ...
  while(!this->midi_signals_handled.empty() && itteration < MAX_WHILE_ITTERATIONS){
    ...
    midi_CC_update_available[(uint8_t)(tmp>>8)][(uint8_t)(tmp)] = 0;
    this->midi_signals_handled.pop_back();
    itteration++;
  }
}

void hidInterface::force_clear_midi_CC_update_available(key * inputKey)
{
  midi_CC_update_available[inputKey->MIDI_channel][inputKey->MIDI_data1] = 0;
}
...
void handleControlChange(byte channel, byte number, byte value){
  ...
  midi_CC_state_storage[channel][number] = value;
